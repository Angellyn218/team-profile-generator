...
function renderEmployees(team) {
    ...
    for (let i = 0; i < team.length; i++) {
        ...
        if (team[i].getRole() === "Manager") {
            const third = `Office Number: ${team[i].officeNumber}`;
        } else if (team[i].getRole() === "Engineer") {
            const third = `Github: <a href="${team[i].getGithub()}" class="card-link">${team[i].getGithub()}</a>`;
        } else {
            const third = `School: ${team[i].school}`
        }
        ...
        <div class="card-body">
            <ul class="list-group list-group-flush">
                ...
                <li class="list-group-item">Email: <a href="${team[i].getEmail()}" class="card-link">${team[i].getEmail()}</a></li>
